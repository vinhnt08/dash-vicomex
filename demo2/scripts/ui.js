const UI = {
  ...
  statCard({ label, value, unit, sub, subType }) {
    const subEl = sub
      ? `<span class="body-sm metric-card__note${subType === 'success' ? ' metric-card__note--success' : ''}">${sub}</span>`
      : '';
    return `
  ...
  driverRow(item, idx) {
    ...
    const surchargeClass = item.highlight ? 'salary-badge salary-badge--warning' : 'salary-badge';
    return `
      <tr ${highlight} style="animation-delay:${idx * 50}ms">
        ...
        <td><span class="${surchargeClass}">${item.surcharge} VNĐ</span></td>
  ...
  routeSuggestionCard(route) {
    return `
      <div class="card route-suggestion">
